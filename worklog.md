...
- OPS NOTE (post-handover): repeated `next-server` crashes were diagnosed as the KERNEL OOM KILLER — several overlapping dev-server instances accumulated after the .env-reload restarts (dmesg: next-server killed, anon-rss 2.1GB on a 4GB box). Fixed by pkill-ing ALL bun/next instances and starting exactly ONE `bun run dev` (subshell background). Memory now ~1.9GB/4GB and stable across tool calls. Rule for future rounds: ALWAYS `ps aux | grep -E "bun run|next"` and kill duplicates before starting the server.
